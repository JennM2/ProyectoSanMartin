...
const Students = () => {
  ...
  const loadDataQualification = (id) => {
    Axios.get(`http://localhost:3001/api/programming/notesByIdStudent/${id}`).then((response) => {
      let dataYears = [];
      let dataToogles = [];
  ...
  const handleSave = () => {
    ...
    Axios.post("http://localhost:3001/api/students/", newRow)
      .then((res) => {
  ...
  const handleEditSave = () => {
    if (editingRow) {
      ...
      Axios.put(`http://localhost:3001/api/students/${userStudent}`,newData)
        .then(()=>{
