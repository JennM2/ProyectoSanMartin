...
const Secretaries = () => {
  ...
  const loadData = () => {
    Axios.get("http://localhost:3001/api/secretaries")
      .then((res) => {
  ...
  const handleSave = () => {
    ...
    Axios.post("http://localhost:3001/api/secretaries", newRow)
      .then((res) => {
        loadData();
  ...
  const handleFinalDeletion = () => {
    Axios.delete(`http://localhost:3001/api/secretaries/disable/${idSpecific}`)
      .then(() => {
  ...
  const handleEditSave = () => {
    if (editingRow) {
      ...
      Axios.put(
        `http://localhost:3001/api/secretaries/${editedId}`,
        editedRowData
      )
        .then((res) => {
          loadData();
