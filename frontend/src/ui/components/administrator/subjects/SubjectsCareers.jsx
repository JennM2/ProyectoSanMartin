...
const SubjectsCareers = () => {
  ...
  const loadData = () => {
    Axios.get("http://localhost:3001/api/subjects/subjects")
      .then((response) => {
  ...
  useEffect(() => {
    if (selectedCareer) {
      Axios.get(`http://localhost:3001/api/careers/year/${selectedCareer}`)
        .then((response) => {
  ...
  const handleSave = () => {
    ...
    Axios.post("http://localhost:3001/api/subjects/subjects", newSubject)
      .then((res) => {
  ...
  const handleFinalDeletion = () => {
    Axios.put(
      `http://localhost:3001/api/subjects/inactivate/${idDeleteSpecific}`
    )
      .then(loadData)
      .then(() => {
        setIsDeleteDialog(!isDeleteDialog);
  ...
  const handleEditSave = () => {
    if (editingRow) {
      ...
      Axios.put(
        `http://localhost:3001/api/subjects/subjects/${editedId}`,
        editedRowData
      )
        .then((res) => {
          loadData();
