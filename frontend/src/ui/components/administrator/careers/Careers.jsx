...
const Careers = () => {
  ...
  const loadData = () => {
    Axios.get("http://localhost:3001/api/careers")
      .then((response) => {
  ...
  useEffect(() => {
    const fetchSubjects = (setDataFunction) => {
      if (idCareer) {
        Axios.get(
          `http://localhost:3001/api/subjects/career/primero?idCareer=${idCareer}`
        )
          .then((response) => {
  ...
  useEffect(() => {
    const fetchSubjects = (setDataFunction) => {
      if (idCareer) {
        Axios.get(
          `http://localhost:3001/api/subjects/career/segundo?idCareer=${idCareer}`
        )
          .then((response) => {
            const dataArray = response.data.map((subject, index) => [
              index + 1,
  ...
  useEffect(()=>{
    const fetchSubjects = (setDataFunction) => {
      if (idCareer) {
        Axios.get(
          `http://localhost:3001/api/subjects/career/tercero?idCareer=${idCareer}`
        )
          .then((response) => {
            const dataArray = response.data.map((subject, index) => [
              index + 1,
  ...
  const handleSave = () => {
    ...
    Axios.post("http://localhost:3001/api/careers", newRow)
      .then((res) => {
  ...
  const handleEditSave = () => {
    if (editingRow) {
      ...
      Axios.put(`http://localhost:3001/api/careers/${editedId}`, editedRowData)
        .then((res) => {
          loadData();
