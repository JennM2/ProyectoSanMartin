...
const Subjects = () => {
    ...
    const columns = ['N°', 'Materia', 'Carrera', 'Año', 'Docente', 'Horario', 'Grupo', "Mes", 'Fecha de habilitación', 'Fecha de Finalización', 'Estado', 'Acción'];
    ...
    const loadData = () => {
        Axios.get("http://localhost:3001/api/enable")
          .then((res) => {
    ...
    useEffect(() => {
        Axios.get(
          `http://localhost:3001/api/subjects?carrera=${career  || 'default'}&year=${year  || 'default'}`
        )
          .then((response) => {
    ...
    useEffect(()=>{
        ...
        Axios.get(
            `http://localhost:3001/api/subjectTeacher/subjectTeacher/${subject || 'default'}`
          )
            .then((response) => {
    ...
    const handleFinalDeletion = () => {
        Axios.delete(`http://localhost:3001/api/enable/${idSpecific}`).then(()=>{
    ...
    const handleSave = () => {
        ...
        Axios.post(`http://localhost:3001/api/enable`,newRow).then(()=> {
    ...
    const handleEditClick = (rowId) => {
        ...
        setSchedule(row[5]);
        setGroup(row[6]);
        setMonth(`${row[7].slice(-4)}-${
            ((a = (['Enero', 'Febrero', 'Marzo', 
            'Abril', 'Mayo', 'Junio', 
            'Julio', 'Agosto', 'Septiembre', 
            'Octubre', 'Noviembre', 'Diciembre'].findIndex(item=>item===(row[7].slice(0,(row[7].indexOf('/') - 1)))) + 1))=>{return a>=10? a : `0${a}`})()}`);
        setEnablementDate(row[8]);
        setDisablementDate(row[9]);
    ...
    const handleEditSave = () => {
        ...
        Axios.put(`http://localhost:3001/api/enable/${editedId}`,updateData).then(()=>{
            enqueueSnackbar('Editado',{variant:'success'});
            loadData();
    ...
    const loadDataStudents = async(idEnable) => {
        Axios.get(`http://localhost:3001/api/programming/allStudents/${idEnable}`).then(response=>{
                    ...
                    <div className={modalClasses.content}>
                        ...
                        <div className={classes.contentEnableSubject}>
                            ...
                            <div className={classes.contentRight}>
                                ...
                                <div className={classes.dataEnable}>
                                    <label className={classes.labelSelect} htmlFor="opciones">Mes / Gestion:</label>
