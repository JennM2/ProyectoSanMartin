...
const Report = () => {
    ...
    const loadStudents = () =>{
        ...
        if(searchTermSubject){
            ...
            Axios.get(`http://localhost:3001/api/programming//allStudents/${searchTermSubject}`).then(response => {
    ...
    const loadDataSubjects = () => {
        if(searchTermMonth){
        Axios.get(`http://localhost:3001/api/enable//byMonth/${searchTermMonth}/${token.idTeacher}`).then(response => {
