...
const ActiveSubjects = () => {
  ...
  const loadData = () => {
    Axios.get(`http://localhost:3001/api/enable/teacher?teacherId=${token.idTeacher}`).then(response => {
  ...
  const loadDataStudents = async(id) => {
    const response = await Axios.get(`http://localhost:3001/api/programming/allStudents/${id}`);
  ...
  const handleSave = () => {
    ...
    Axios.put('http://localhost:3001/api/programming/updateNotes', editData).then(()=>{
  ...
  const handleFinishEnable = async () => {
    ...
    Axios.put(`http://localhost:3001/api/enable/finish/${subject.idEnable}`).then(()=>{
