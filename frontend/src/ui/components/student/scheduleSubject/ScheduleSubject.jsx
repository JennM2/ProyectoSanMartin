...
const ScheduleSubject = () => {
  ...
  const loadData = () => {
    Axios.get(`http://localhost:3001/api/enable/toProgram/${token.idStudent}`).then(response=>{
  ...
  const handlePayment = () => {
    ...
    setPayProcess(true);
    Axios.post('http://localhost:3001/api/programming',dataPayProgramming).then(()=>{
      setPayProcess(false);
