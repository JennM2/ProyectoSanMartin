...
const login = async (req, res) => {
    ...
    try {
        let [results] = await req.db.promise().query('SELECT idUser, rol, stateUser FROM users WHERE user=? and password=?', [user, password]);
        results = results[0];
        console.log(results);
        ...
        results = await req.db.promise().query(sql, [user, password]);
        console.log(results);
