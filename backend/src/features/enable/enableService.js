    ...
    try {
        const query = `
            SELECT 
                ...
                s.year,
                e.groupe,
                CONCAT(u.paterno, ' ', u.materno, ' ', u.names) AS teacher,
                e.schedule,
                e.month,
                e.dateStart,
                e.dateEnd,
...
const enableSubject = async (req, res) => {
    try {
        const {idTeaSub, subjectName, dateStart, dateEnd, schedule, group} = req.body;
        ...
        const errors = (checkFields({
                        ...
                        Horario : schedule,
                        Grupo : group,
                        Mes_Gestion : req.body.month},res))
        if(errors)
        ...
        const teaSubQuery = 'SELECT idTeaSub FROM teachers_subjects WHERE idSubject = ?';
        const [teaSubResults] = await req.db.promise().query(teaSubQuery, [idSubject])
        ...
        const checkEnableQuery = 'SELECT * FROM enable WHERE idTeaSub = ? AND month = ? AND stateEnable = ? AND schedule = ? AND groupe = ?' ;
        const [checkResults] = await req.db.promise().query(checkEnableQuery, [idTeaSub, month, stateEnable, schedule, group]);
            if (checkResults && checkResults.length > 0) {
                return res.status(400).json({ message: 'Ya existe una habilitacion con los datos' });
            }

        // Paso 4: Insertar la habilitación en la tabla enable
        const insertQuery = 'INSERT INTO enable (idTeaSub, month, dateStart, dateEnd, schedule, stateEnable, scoreQ1, scoreQ2, scoreQ3, scoreQ4, numEvaluations, groupe, stateSubject) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
        await req.db.promise().query(insertQuery, [idTeaSub, month, dateStart, dateEnd, schedule, stateEnable, 0, 0, 0, 0, 0, group, 'En progreso']);
...
const updateEnable = async (req, res) => {
    try {
        ...
        const updateQuery = 'UPDATE enable SET idTeaSub = ?, month = ?, dateStart = ?, dateEnd = ?, schedule = ?, stateEnable = ?,scoreQ1 = ?, scoreQ2 = ?, scoreQ3 = ?, scoreQ4 = ?, numEvaluations = ? WHERE idEnable = ?';
        const updateResults = await req.db.promise().query(updateQuery, [idTeaSub, month, dateStart, dateEnd, schedule, stateEnable, 0, 0, 0, 0 , 0, idEnable])
        // Verificar si se actualizó correctamente
        if (updateResults.affectedRows === 0) {
            return res.status(404).json({ message : 'La habilitación especificada no existe' });
        }
